...
async function run() {
  ...
  // Читання після тайм-ауту запуску повторюється ОДИН раз у межах бюджету: скрипт
  // зазвичай виконується за секунди, а тайм-аут — це зависання на боці Google.
  calls = 0;
  global.fetch = async () => {
    calls += 1;
    throw abortError();
  ...
  assert.equal(calls, 2, "читання після тайм-ауту пробуємо ще раз, але не більше");
