...
const WRITE_ACTIONS = new Set([
  ...
  "file_upload_init", "file_upload_chunk", "file_trash",
...
// звичайний запис. 55 с вкладаються в стандартну тривалість функцій проєкту (fluid
// compute; читання з тайм-аутом 40 с уже працюють у проді). Блок functions/maxDuration
// у vercel.json НЕ додавати — з ним розгортання падає.
const LONG_TIMEOUTS_MS = {
  file_upload_chunk: 55000,
  file_upload_status: 55000,
  contractor_send: 55000,
  contractor_send_file: 55000,
};
...
async function callAdminSheets(action, payload) {
  ...
  // Надсилання підряднику з request_id Apps Script теж впізнає і не дублює.
  const retryableContractor = (action === "contractor_send" || action === "contractor_send_file") && !!(
    payload && String(payload.request_id || "").trim()
  );
  const retryableWrite = retryablePayment || retryableOrder || retryableContractor;
  const readAction = !WRITE_ACTIONS.has(action);
  const retryableAction = readAction || retryableWrite;
  const longAction = Object.prototype.hasOwnProperty.call(LONG_TIMEOUTS_MS, action);
  const timeoutMs = longAction ? LONG_TIMEOUTS_MS[action] : (readAction ? READ_TIMEOUT_MS : WRITE_TIMEOUT_MS);
  ...
  try {
    ...
    // Після повного тайм-ауту не повторюємо: для читання це ще одна довга операція, а
    // для довгих дій другий запит вийшов би за ліміт функції Vercel — повторює клієнт.
    if (!retryableAction || ((readAction || longAction) && err && err.code === "SHEETS_TIMEOUT")) throw err;
