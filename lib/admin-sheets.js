...
async function callAdminSheets(action, payload) {
  ...
  // Читання можна повторити завжди. Із записів повторюємо лише add_payment, коли
  // клієнт передав request_id: Apps Script впізнає його й не створить дубль.
  ...
  async function sendWithReadableTimeout() {
    try {
      ...
      if (err && err.name === "AbortError") {
        const timeoutError = new Error("Google Sheets не відповів за 25 секунд");
  ...
  try {
    ...
    if (!retryablePayment) throw err;
    await new Promise((r) => setTimeout(r, 800));
    res = await sendWithReadableTimeout();
  }
  if (!res.ok && (!WRITE_ACTIONS.has(action) || retryablePayment)) {
