...
function groupPaymentMetrics(group) {
  ...
  // Комісію утримує підрядник, тому переказати він має валовий прибуток МІНУС
  // комісію. Без комісії (переважна більшість замовлень) це і є валовий прибуток.
  const marginDue = hasValue(source.margin_due)
    ? amount(source.margin_due)
    : (profit > 0 ? Math.max(0, profit - Math.max(0, amount(source.commission))) : profit);
  ...
  return {
    clientSettled,
    marginDue,
    marginReady: clientSettled ? marginDue : 0,
    marginReceived,
