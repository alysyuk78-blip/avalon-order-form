...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      // Файл до 3 МБ — одним запитом: без окремого відкриття сесії й частин. Кожне
      // звернення до Apps Script може зависнути на боці Google на кілька секунд, тож
      // скріншот чи PDF із двох запитів перетворюється на один.
      if (action === "small") {
        const data = String(src.data || "");
        if (!data) return res.status(400).json({ error: "Порожній файл" });
        if (data.length > MAX_CHUNK_BASE64) return res.status(413).json({ error: "Файл завеликий для одного запиту" });
        const out = await callAdminSheets("file_upload_small", {
          order_number: orderNumber,
          name: String(src.name || "").slice(0, 200),
          mime: String(src.mime || "").slice(0, 150),
          size: Number(src.size) || 0,
          data,
          request_id: String(src.request_id || "").trim().slice(0, 120),
        });
