...
const DIAG = {
  ...
  contractor_failed: {
    title: "⚠️ <b>Замовлення не надіслалось підряднику</b>\nСпроба створити/оновити гілку в групі не вдалася.",
    cause: "Найчастіше — бот втратив право «Керувати гілками» в групі підрядника, або Telegram тимчасово недоступний.",
    fix: "1) Перевір, чи статус у таблиці справді «В роботі».\n2) Група підрядника → Адміністратори → бот → увімкни «Керувати гілками».\n3) Якщо потрібно — поверни статус на «Нове» і знову постав «В роботі»."
  }
};
...
function buildPrompt(type, body, cdns) {
  ...
  if (type === "contractor_failed") {
    return base + "Спроба надіслати замовлення підряднику не спрацювала (Apps Script google-apps-script-v2.js, createOrderTopic_). Перевір фактичний статус у таблиці, права бота в групі (Manage Topics), getMe/createForumTopic, логіку onEditDelivery і маркер thread_. Деталі: " + (det || "(немає)");
  }
