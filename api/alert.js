...
const DIAG = {
  ...
  contractor_failed: {
    title: "⚠️ <b>Замовлення НЕ дійшло підряднику</b>\nСтатус «В роботі» поставлено, але гілка в групі не створилась.",
    cause: "Найчастіше — бот втратив право «Керувати гілками» в групі підрядника, або Telegram тимчасово недоступний.",
    fix: "1) Група підрядника → Адміністратори → бот → увімкни «Керувати гілками».\n2) Постав статус «В роботі» ще раз — надсилання повториться."
  }
};
...
function buildPrompt(type, body, cdns) {
  ...
  if (type === "contractor_failed") {
    return base + "Замовлення з підтвердженим статусом «В роботі» не доходять у групу підрядника (Apps Script google-apps-script-v2.js, createOrderTopic_). Перевір права бота в групі (Manage Topics), getMe/createForumTopic, та логіку onEditDelivery і маркера thread_. Деталі: " + (det || "(немає)");
  }
