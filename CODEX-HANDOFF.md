...
## Додатковий fix для порожнього `AF`

Після чергового тесту користувач повідомив, що посилання в `AF` взагалі відсутнє.
Щоб відрізнити “файл не обрали” від “файл був у формі, але не дійшов до Apps
Script”, Vercel API тепер передає в Apps Script `pattern_file_meta`:
- `name`;
- `type`;
- `size`;
- `data_length`.

Якщо `pattern_file_meta.name` є, але `data.pattern_file` не збережений у Drive,
Apps Script пише в `AF`: `Файл візерунку НЕ збережено: файл був у формі/Vercel,
але Apps Script не отримав base64-дані`.
...
## Прибрано зайві Drive-алерти

Після тесту fallback працював частково: `AF` містив і стару Drive-помилку, і
новий маркер `збережено в Telegram`, а власнику приходив лякаючий alert
`Замовлення НЕ дійшло підряднику`. Це був шум від `DriveApp.createFile`, не
реальна помилка Telegram fallback.

Зміни:
- Apps Script більше не шле owner alert лише через помилку Drive-збереження;
- коли Vercel прикріплює Telegram `file_id`, Apps Script прибирає з `AF` старі
  рядки `Файл візерунку НЕ збережено...`;
- у `AF` лишається нормальний статус: файл збережено в Telegram і буде
  надіслано підряднику.
